...
%     if size(avp0,2)>10, avp0=avp0(:,[1:9,end]); end
    switch ptype
        case {'A', 'a', 'y'},
            ...
            subplot(223), plot(t, yawplot(avp(:,3)/glv.deg)), xygo('y'); legend('Yaw Ref.');
        case 'av',
            avp = avp0; t = avp(:,end);
...
%                 subplot(122), hold on, plot(t, err(:,1:2)/glv.min, strk, 'LineWidth',2); xygo('mu'); mylegend('mux','muy');
        ...
        case {'a', 'y'},
            for k=1:kk
                ...
                subplot(221), hold on, plot(t, avp(:,1:2)/glv.deg, strk, 'LineWidth',2), xygo('pr'); %legend('Pitch','Roll');
                %subplot(223), hold on, plot(t, avp(:,3)/glv.deg, strk, 'LineWidth',2), xygo('y'); legend('Yaw');
                subplot(223), hold on, plot(t, yawplot(avp(:,3)/glv.deg), strk, 'LineWidth',2), xygo('y'); %legend('Yaw');
